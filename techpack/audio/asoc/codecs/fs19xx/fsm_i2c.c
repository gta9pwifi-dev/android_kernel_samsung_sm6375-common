...
//+P86801AA1, zhangtao10.wt, ADD, 2022/05/09, add smartpa info
			...
			const struct i2c_device_id *id)
{
	...
	pr_debug("enter");
//+P86801AA1, zhangtao10.wt, ADD, 2023/05/13, smartpa compatibility
	if ((smartpa_type == AW88266) || (smartpa_type == AW88257)) {
		pr_info("%s awinic device has been registered\n",__func__);
		return EEXIST;
	}
//-P86801AA1, zhangtao10.wt, ADD, 2023/05/13, smartpa compatibility
...
//+P86801AA1, zhangtao10.wt, ADD, 2023/05/13, smartpa compatibility
	i2c->addr ^= (3 << 2);
	dev_info(&i2c->dev, "fsm_i2c_probe addr=0x%x", i2c->addr);
//-P86801AA1, zhangtao10.wt, ADD, 2023/05/13, smartpa compatibility
...
//+P86801AA1, zhangtao10.wt, ADD, 2022/05/09, add smartpa info
	if (true == fsm_dev->is19xx) {
		pr_info("%s audiock fsm detect device seccess\n",__func__);
		smartpa_type =snd_soc_set_smartpa_type("fsm_i2c_probe", FS16XX);
	} else {
